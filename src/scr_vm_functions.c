...
// Begin OpenCJ extra PlayerCmds
int clfps[MAX_CLIENTS][20];
int clfpsindex = 0;
...
void PlayerCmd_ResetFPS(scr_entref_t arg)
{
	...
	else
	{
        ...
        {
            const int maxNrFps = (sizeof(clfps[0]) / sizeof(clfps[0][0]));
...
void PlayerCmd_GetFPS(scr_entref_t arg)
{
	...
	else
	{
        ...
        else
        {
            ...
            for (int i = 0; i < maxNrFps; i++)
            {
                if (clfps[entityNum][i] == -1)
                {
                    Scr_AddInt(-1);
                    return;
                }
                total += clfps[entityNum][i];
                Scr_AddInt(total);
            }
        }
	}
...
void PlayerCmd_FPSNextFrame()
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (!gentity->client)
        {
		    clfps[i][clfpsindex] = 0;
			continue;
		}

		client_t *client = svs.clients + i;
		clfps[i][clfpsindex] = client->clFrames;
		client->clFrames = 0;
	}

	if (++clfpsindex >= maxNrFps)
    {
		clfpsindex = 0;
    }
}
